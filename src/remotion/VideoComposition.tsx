...
function renderSectionScene(
  ...
  switch (section.sceneType) {
    case 'diagram':
      return <DiagramScene section={section} index={index} total={total} />
    case 'split':
      return <SplitScene section={section} index={index} total={total} />
    case 'text':
    default:
      return <SectionScene section={section} index={index} total={total} />
  }
}

export const VideoComposition: React.FC<ProcessedVideoData> = (props) => {
  const { topic, summary, sections } = props
    ...
    <AbsoluteFill style={{ backgroundColor: '#0f0f23' }}>
      <TransitionSeries>
        ...
        {sections.flatMap((section, index) => [
          ...
          >
            {renderSectionScene(section, index, sections.length)}
          </TransitionSeries.Sequence>,
        ])}
